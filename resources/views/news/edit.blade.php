@extends('layouts.app')
@section('title', 'Hír szerkesztése')
@section('content')
    ...
    <section id="content">
        <div class="container">
            ...
            <div class="col">
                <h3 class="mb-4">Hír szerkesztése</h3>
                <form action="{{ route('admin.news.update', $news->id) }}" method="post" class="row">
                    ...
                    <div class="form-group col-6">
                        ...
                        <input type="text" class="form-control" name="slug" id="slug" value="{{ $news->slug }}" readonly>
                        @error('slug') <span class="text-sm text-danger">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group col-12">
                        <label for="body">Szöveg</label>
                        <textarea type="text" name="body" id="body">{{ $news->body }}</textarea>
                        @error('body') <span class="text-sm text-danger">{{ $message }}</span> @enderror
                    </div>

                    <div class="form-group col-12">
                        <button class="button button-reveal button-medium button-rounded tright"><i class="icon-angle-right"></i><span>Mentés</span></button>
                    </div>
                </form>
            </div>
        </div>
